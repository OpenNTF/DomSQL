...
//		b.append(".");
	...
	void createAllType(Database db, int index) throws NotesException {
		...
		try {
			...
			{
				int nFields = (int)(Math.random()*6); // [0-6 items]
				for(int i=0; i<nFields; i++) {
					mx.add("text_"+sIndex+"_"+(i+1));
				}
			}
			...
			Vector<Object> md = new Vector<Object>();
			{
				int nFields = (int)(Math.random()*6); // [0-6 items]
				for(int i=0; i<nFields; i++) {
					md.add(createDate(session, 2010, i+1, index));
				}
			}
			...
			Vector<Object> mt = new Vector<Object>();
			{
				int nFields = (int)(Math.random()*6); // [0-6 items]
				for(int i=0; i<nFields; i++) {
					mt.add(createTime(session, 6, i+1, index));
				}
			}
			...
			Vector<Object> mdt = new Vector<Object>();
			{
				int nFields = (int)(Math.random()*6); // [0-6 items]
				for(int i=0; i<nFields; i++) {
					mdt.add(createDateTime(session, 2011, i+1, index, 6, i+1, index));
				}
			}
			...
			if(false) { // DateTime range do not work with multiple values?
				Vector<Object> mrg = new Vector<Object>();
				{
					int nFields = (int)(Math.random()*6); // [0-6 items]
					for(int i=0; i<nFields; i++) {
						mrg.add(createDateTimeRange(session, 2012, 2, index, 4, i+1, index));
					}
				}
